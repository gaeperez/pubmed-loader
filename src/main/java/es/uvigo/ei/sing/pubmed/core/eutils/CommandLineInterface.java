...
import es.uvigo.ei.sing.pubmed.core.eutils.util.OutputListener;
...
public final class CommandLineInterface implements OutputListener {
    ...
    public static void commandLineStart(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (validParameter(args[i].trim())) {
                StringBuilder user_params = new StringBuilder();
     ...
     */
    private static void setFilePaths(CommandLineInterface m) throws IOException {
    ...
    public static int search(String needle, String... haystack) {
        for (int i = 0; i < haystack.length; i++) {
            if (haystack[i].trim().contains(needle)) {
                return i;
            }
        }
        return -1;
     ...
     */
    private static boolean validParameter(String param) {
        for (String ARGUMENT : ARGUMENTS)
            if (param.equals(ARGUMENT)) return true;
        return false;
    }
