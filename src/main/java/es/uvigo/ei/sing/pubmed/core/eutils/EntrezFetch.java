package es.uvigo.ei.sing.pubmed.core.eutils;
/*
  eFetch.java
  <p>
...
//        setEmail("gregcope@algosome.com");
     ...
     */
    public EntrezFetch(Entrez search) throws IllegalArgumentException {
        super(search);
        setURL("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi");
     ...
     * turn that feature off and/or writeToDisk the output to a file.  This function should not be called directly, but rather through doQuery.
     *
     ...
     * @see JeUtils for directions to set output flags.
     */
    public void parseInput(InputStream is) throws IOException {
        if (DEBUG) {
            log.debug("Processing eFetch...");
        }
...
//                        System.out.println(line);
        ...
        if (getUseHistory().equals("y")) {
            if (getQueryKey() != null && getWebEnv() != null) {
                for (String st : filter) {
                    ...
                    if (s.indexOf("id=") == 0 || s.indexOf("usehistory=") == 0 || s.indexOf("term=") == 0)
                        continue;//don't do ID's when we use history.
                    appender.append(s);
                    appender.append("&");
                }
            }
            ...
            for (String st : filter) {
                String s = st.replaceAll("-", "");
                if (isESearchSpecificParam(s.substring(0, s.indexOf("=")))) continue;
                if (s.indexOf("WebEnv=") == 0 || s.indexOf("query_key=") == 0 || s.indexOf("term=") == 0 || s.indexOf("usehistory=") == 0)
                    continue;//don't do ID's when we use history.
                appender.append(s);
                appender.append("&");
            }
     ...
     */
    @Override
    public void doQuery(InputStreamParser is) throws IOException {
        if (getIds() == null) {
